...
async function doAnalyze() {
  ...
  result.value = null
  try {
    result.value = await api.analyzeAsset(trimmed, 'fund', name.value)
  } catch (e: any) {
...
async function doLookupName() {
  const trimmed = code.value.trim()
  if (!trimmed || nameLoading.value) return
...
function openAddDialog() {
  addTxType.value = 'buy'
  addShares.value = 1000
  addCost.value = result.value?.market_data?.[result.value.market_data.length - 1]?.close || 1
  addFee.value = 0
...
<template>
  <div class="page">
    ...
    <div v-if="result && result.market_data && result.market_data.length > 0" class="card mt-4 chart-section">
      <h3 class="section-title">{{ result.code }} {{ result.name }} - 历史净值走势</h3>
      <FinancialChart :data="result.market_data" asset-type="fund" :height="420" />
    </div>
    ...
    <div v-if="!result && !loading && !error" class="empty-state card">
      <p class="text-secondary">输入基金代码开始分析</p>
    </div>
