...
const dialogShares = ref(100)
const dialogPrice = ref(10)
...
function openTxDialog(mode: 'buy' | 'add' | 'reduce', item?: PortfolioItem) {
  ...
  if (mode === 'buy') {
    dialogCode.value = ''
    dialogName.value = ''
    dialogPrice.value = 10
    dialogShares.value = 100
  } else if (item) {
...
const items = computed<PortfolioItem[]>(() => {
  ...
  return portfolio.value.stocks.items
})
...
const totalFees = computed(() => {
  ...
  return portfolio.value.stocks.items.reduce((s, i) => s + i.total_fees, 0)
})

const netAsset = computed(() => {
  if (!portfolio.value) return 0
  return portfolio.value.stocks.total_market_value - totalFees.value
})
...
async function load() {
  ...
  try {
    ...
    // 预加载交易记录数量
    if (portfolio.value) {
      await Promise.all(portfolio.value.stocks.items.map(item =>
...
<template>
  <div class="page">
    ...
    <!-- Summary bar -->
    <div v-if="stockSummary && items.length > 0" class="summary-bar">
      <div class="summary-stat">
        <span class="stat-label">总成本</span>
        <span class="stat-val">¥{{ money(stockSummary.total_cost) }}</span>
      </div>
      <div class="summary-stat">
        <span class="stat-label">总市值</span>
        <span class="stat-val">¥{{ money(stockSummary.total_market_value) }}</span>
      </div>
      <div class="summary-stat">
        <span class="stat-label">盈亏</span>
        <span class="stat-val" :class="stockSummary.total_profit_loss >= 0 ? 'clr-up' : 'clr-down'">
          {{ stockSummary.total_profit_loss >= 0 ? '+' : '' }}{{ pct(stockSummary.total_profit_loss_pct) }}%
        </span>
      </div>
      <div class="summary-stat">
      ...
      <div v-for="item in items" :key="item.id" class="pos-card card">
        ...
        <div class="pos-stats">
          <div class="ps-item">
            ...
            <span class="ps-value">{{ item.shares }}股</span>
          </div>
          <div class="ps-item">
            <span class="ps-label">成本价</span>
            <span class="ps-value">¥{{ money(item.cost_price) }}</span>
          </div>
          <div class="ps-item">
            <span class="ps-label">现价</span>
          ...
          <div v-else class="tx-list">
            <div v-for="tx in txCache[item.id]!" :key="tx.id" class="tx-row">
              ...
              <template v-else>
                ...
                <span class="tx-shares">{{ tx.shares }}股</span>
    ...
    <div v-if="items.length === 0 && !loading" class="card empty-state mt-4">
      <p class="clr-secondary">暂无股票持仓</p>
        ...
        <div class="modal-card card">
          ...
          <div class="modal-bd">
            <p v-if="txDialogMode !== 'buy' && txDialogItem" class="modal-desc">
              ...
              <span v-if="txDialogMode === 'reduce'">（当前持仓 {{ txDialogItem.shares }} 股）</span>
            </p>
            <div class="dlg-form">
              <template v-if="txDialogMode === 'buy'">
                <div class="field">
                  <label>股票代码</label>
                  <input v-model="dialogCode" type="text" placeholder="如 000001" class="form-input" @input="onCodeInput" />
              ...
              <div class="field">
                ...
                <span v-if="txDialogMode === 'reduce' && txDialogItem" class="field-hint">最大 {{ txDialogItem.shares }} 股</span>
              </div>
              <div class="field">
                <label>单位净值</label>
          ...
          <div v-else-if="modalAnalysis" class="modal-bd">
            ...
            <template v-if="showProfessional">
              <div v-if="chartData.length > 0" class="chart-section">
                <h3 class="sect-title">K线走势</h3>
                <FinancialChart :data="chartData" asset-type="stock" :height="400" />
              </div>
              ...
              <div class="grid-2">
                <div class="card-inner"><h3 class="sect-title">技术面</h3><p class="sect-body">{{ modalAnalysis.ai_report.technical_view || '暂无数据' }}</p></div>
                <div class="card-inner"><h3 class="sect-title">基本面</h3><p class="sect-body">{{ modalAnalysis.ai_report.fundamental_view || '暂无数据' }}</p></div>
              </div>
              <div class="grid-2">
